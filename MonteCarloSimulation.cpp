...
Monte_carlo_results monte_carlo_call_put_price(int num_sims, double S, double K, double r, double sigma, double T, std::mt19937& gen) {
    ...
    //beta calculation
    double X_mean = (call_sum / num_sims) * Discount;           // call seq already discounted??
    ...
    result.call_cv_std_error = var_CV;
...
#pragma omp single
    ...
    std::cout << "Call SE:    " << res.call_std_error << "\n";
    std::cout << "Call CI:    [" << res.call_ci_low << ", " << res.call_ci_high << "]\n";

    std::cout << "CV Call:    " << res.call_cv << "\n";
    std::cout << "Var Red:    " << res.variance_reduction << "\n";
